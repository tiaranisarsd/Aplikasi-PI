...
const TentangKegiatanAdmin = () => {
    ...
    const deleteTentangKegiatan = async (tentangKegiatanId) => {
        try {
            await axios.delete(`http://194.59.165.159:5000/tentangKegiatan/${tentangKegiatanId}`);
            getTentangKegiatan();
        } catch (error) {
                ...
                <tbody>
                    {tentangKegiatan.map((tentangKegiatan, index) => (
                        <tr key={tentangKegiatan.id}>
                            ...
                            <td>
                            <a href={`http://194.59.165.159:5000/uploads/tentangKegiatan/${tentangKegiatan.image}`} target="_blank" rel="noopener noreferrer">
